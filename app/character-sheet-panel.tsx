...
function PsionicsGeneration({ character, updateCharacter, canEdit, gmOverride }: { character: Character; updateCharacter: (patch: Partial<Character>) => void; canEdit: boolean; gmOverride: boolean }) {
  ...
  return <section className="psionics-generation-step">
    ...
    {psionics.enabled && <details className="psionics-generation-editor" open>
      <summary><span>Psionic setup</span><small>Choose {modeLimit} attacks · {defenseLimit - 1} additional defenses · {disciplinesAvailableNow} of {totalDisciplines} disciplines available at level {character.level}</small></summary>
      <div>
        ...
        <button type="button" className="primary-button" disabled={!canEdit || psionics.attackModes.length !== modeLimit || psionics.defenseModes.length !== defenseLimit || psionics.disciplines.length !== disciplinesAvailableNow} onClick={() => set({ determination: "established" })}>Establish psionics</button>
      </div>
    </details>}
  </section>;
}
  ...
  return (
    <details className="character-spell-panel">
      ...
      <div className="spell-track-stack">
        ...
        {psionics.enabled && <section className="spell-track psionic-reference-track"><header className="spell-track-header"><span><b>Psionic disciplines</b><small>Strength {psionics.psionicStrength} · Ability {psionics.originalPsionicAbility} · AP {psionics.currentAttackPoints}/{psionics.maxAttackPoints} · DP {psionics.currentDefensePoints}/{psionics.maxDefensePoints}</small></span><span className="spell-capacity-tiles"><i>ATK <b>{psionics.attackModes.length}</b></i><i>DEF <b>{psionics.defenseModes.length}</b></i></span></header><div className="psionic-spell-reference"><section><h3>Modes</h3><div className="psionic-known-modes"><span><b>Attack modes</b><span>{psionics.attackModes.length ? psionics.attackModes.map((mode) => <PsionicAttackModeTooltip mode={mode} key={mode}>{mode}</PsionicAttackModeTooltip>) : "None"}</span></span><span><b>Defense modes</b><span>{psionics.defenseModes.length ? psionics.defenseModes.map((mode) => <PsionicDefenseModeTooltip mode={mode} key={mode}>{mode}</PsionicDefenseModeTooltip>) : "Mind Blank"}</span></span></div></section><section><h3>Disciplines</h3>{psionics.disciplines.length ? <div className="spellbook-list">{psionics.disciplines.map((discipline) => { const rule = psionicDisciplineRules[discipline.name]; return <article key={discipline.id}><div className="spellbook-row automated"><span className="spell-acquisition">{discipline.category === "minor" ? "Devotion" : "Science"}<small>Mastery {discipline.masteryLevel}</small></span><span><PsionicDisciplineTooltip name={discipline.name}>{discipline.name}</PsionicDisciplineTooltip><small>{rule.cost} · {rule.duration}{rule.restriction ? ` · ${rule.restriction}` : ""}</small></span><small>{rule.status === "incomplete-source" ? "GM procedure required" : rule.status === "gm-adjudicated" ? "GM adjudication" : rule.summary}</small></div></article>; })}</div> : <p className="compact-empty">No disciplines recorded.</p>}</section></div></section>}
      </div>
