      ...
      <div className="npc-stable-list">
        {stableNpcs.length === 0 ? <p className="compact-empty">No persistent NPCs yet.</p> : stableNpcs.map((npc) => {
          ...
          return <details className="npc-stable-row" key={npc.id}>
            <summary><span className="npc-token">{npc.token}</span><span className="npc-stable-identity"><b>{npc.name}</b><small>HP {npc.currentHp}/{npc.maxHp} · AC {npc.armorClass} · Move {npc.movementRate}{activeWeapon ? <> · <WeaponRulesTooltip rulesId={activeWeapon.id}>{activeWeapon.name}</WeaponRulesTooltip></> : npc.noncombatant ? " · Noncombatant" : ` · ${npc.damageExpression}`}</small></span><button type="button" className={selected ? "expedition-toggle selected" : "expedition-toggle"} onClick={(event) => { event.preventDefault(); toggleExpedition(npc.id); }}>{selected ? "On expedition" : "Add"}</button><i aria-hidden>▸</i></summary>
            <div className="npc-stable-editor">
              ...
              <fieldset disabled={!canManage}>
                ...
                <details className="unarmed-override-editor psionics-setup-editor"><summary><span><b>PSIONICS</b><small>Attack/defense pools and known combat modes</small></span><i aria-hidden>▸</i></summary><div>
                  {(() => { const psionics = normalizePsionics(npc.psionics); const updatePsionics = (patch: Partial<typeof psionics>) => { const next = { ...psionics, ...patch }; if (patch.maxAttackPoints !== undefined || patch.maxDefensePoints !== undefined) { next.originalPsionicAbility = Math.max(0, Number(next.maxAttackPoints) || 0) + Math.max(0, Number(next.maxDefensePoints) || 0); next.psionicStrength = Math.max(0, Math.min(Number(next.maxAttackPoints) || 0, Number(next.maxDefensePoints) || 0)); } updateNpc(npc.id, { psionics: normalizePsionics(next) }); }; return <>
                    ...
                    <label>Attack points <PsionicTermInfoButton term="attackPoints" /><input type="number" min="0" disabled={!psionics.enabled} value={psionics.currentAttackPoints} onChange={(event) => updatePsionics({ currentAttackPoints: Number(event.target.value) || 0 })} /></label>
                    ...
                    <label>Defense points <PsionicTermInfoButton term="defensePoints" /><input type="number" min="0" disabled={!psionics.enabled} value={psionics.currentDefensePoints} onChange={(event) => updatePsionics({ currentDefensePoints: Number(event.target.value) || 0 })} /></label>
